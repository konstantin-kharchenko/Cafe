...
<head>
    ...
    <title><fmt:message key="admin.ingredients" bundle="${lang}"/></title>
    ...
    <div class="container">
        <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
            ...
            <form action="${abs}/controller" class="col-12 col-lg-auto mb-3 mb-lg-0 me-lg-3">
                <input type="hidden" name="command" value="search_ingredient">
                <input type="hidden" name="currentPage" value="${currentPage}">
                <input type="search" class="form-control" name="search" value="${search_ingredient.name}"
...
<main>
    ...
    <div style="margin-left: 1.5%; margin-right: 1.5%">
        <div class="row row-cols-4">
            <c:choose>
                <c:when test="${not empty search_ingredient}">
                    <div class="col pb-4">
                        <div class="card" style="width: 25rem;">
                            <div class="card-body">
                                <h5 class="card-title"><fmt:message key="product.name"
                                                                    bundle="${lang}"/>: ${search_ingredient.name}</h5>
                                <p class="card-text"><fmt:message key="product.date"
                                                                  bundle="${lang}"/>: ${search_ingredient.shelfLife}</p>
                                <a href="${abs}/controller?command=delete_ingredient&id_ingredient=${search_ingredient.idIngredient}&currentPage=${currentPage}"
                                   class="btn btn-primary"><fmt:message key="product.delete" bundle="${lang}"/></a>
                                <a href="${abs}/controller?command=go_change_ingredient_page&id_ingredient=${search_ingredient.idIngredient}"
                                   class="btn btn-primary"><fmt:message key="order.change" bundle="${lang}"/></a>
                                <a href="${abs}/controller?command=add_to_administrator_basket&currentPage=${currentPage}&id_ingredient=${search_ingredient.idIngredient}&name=${search_ingredient.name}&shelf_life=${search_ingredient.shelfLife}"
                                   class="btn btn-primary"><fmt:message key="product.add.basket" bundle="${lang}"/></a>
                            </div>
                        </div>
                    </div>
                </c:when>
                    ...
                    <c:forEach items="${ingredients}" var="ingredient">
                        <div class="col pb-4">
                            <div class="card" style="width: 25rem;">
                                <div class="card-body">
                                    <h5 class="card-title"><fmt:message key="product.name"
                                                                        bundle="${lang}"/>: ${ingredient.name}</h5>
                                    <p class="card-text"><fmt:message key="product.shelf.life"
                                                                      bundle="${lang}"/>: ${ingredient.shelfLife}</p>
                                    <a href="${abs}/controller?command=delete_ingredient&id_ingredient=${ingredient.idIngredient}&currentPage=${currentPage}"
                                       class="btn btn-primary"><fmt:message key="product.delete" bundle="${lang}"/></a>
                                    <a href="${abs}/controller?command=go_change_ingredient_page&id_ingredient=${ingredient.idIngredient}"
                                       class="btn btn-primary"><fmt:message key="order.change" bundle="${lang}"/></a>
                                    <a href="${abs}/controller?command=add_to_administrator_basket&currentPage=${currentPage}&id_ingredient=${ingredient.idIngredient}&name=${ingredient.name}&shelf_life=${ingredient.shelfLife}"
    ...
    <nav aria-label="Ad navigation">
        <ul class="pagination mt-2">
            <li class="page-item ${firstPage ? 'disabled' : ''}">
                <a class="page-link"
                   href="${abs}/controller?command=go_all_ingredients&currentPage=${currentPage - 1}"
                   aria-label="Previous">
            ...
            <c:forEach var="i" begin="1" end="${pageCount}">
                <li class="page-item ${currentPage == i ? 'active': ''}">
                    <a class="page-link"
                       href="${abs}/controller?command=go_all_ingredients&currentPage=${i}">${i}</a>
                </li>
            </c:forEach>
