...
<head>
    ...
    <title><fmt:message key="admin.applications" bundle="${lang}"/></title>
                                ...
                                <c:forEach var="administrator" items="${administrators}">
                                    <a class="list-group-item list-group-item-action flex-column align-items-start border border-primary">
                                        <div class="d-flex w-100 justify-content-between">
                                            <h5 class="mb-1"><c:out value="${administrator.login}"/></h5>
                                            <img class="rounded-circle mt-5" width="150px"
                                                 src="data:image/png;base64,${administrator.stringPhoto}">
                                            <small><c:out value="${administrator.name}"/> <c:out
                                                    value="${administrator.surname}"/>
                                            </small>
                                        </div>
                                        <small>
                                            <a class="w-25 btn btn-lg btn-primary m-1"
                                               href="${abs}/controller?command=admin_approval&id_administrator=${administrator.idAdministrator}">
