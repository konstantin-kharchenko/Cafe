...
public class SearchProductCommand implements Command {
    ...
    public Router execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
        ...
        try {
            Optional<Product> optionalProduct = ProductServiceImpl.getInstance().findProductByName(name);
            if (optionalProduct.isPresent()) {
                Product product = optionalProduct.get();
                request.setAttribute(SEARCH_PRODUCT_ATTRIBUTE, product);
                request.setAttribute(CURRENT_PAGE, 1);
                return new Router(PagePath.ALL_PRODUCTS_PAGE);
            } else {
